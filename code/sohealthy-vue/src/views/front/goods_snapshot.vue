...
<script lang="ts" setup>
    ...
    async function loadGoodSnapshot() {
        const snapshotId = router.currentRoute.value.params.id;
        const mode = router.currentRoute.value.params.mode;
        const result = await request.post(`/${mode}/goods/findSnapshot`, { snapshotId });
        const { departmentExam = [], labExam = [], medicalExam = [], otherExam = [] } = result;

        Object.assign(data, {
            ...result,
            coverImage: `${proxy!.$minioUrl}/${result.coverImage}`,
            departmentExam,
            labExam,
            medicalExam,
            otherExam,
            departmentExam_count: departmentExam.length,
            labExam_count: labExam.length,
            medicalExam_count: medicalExam.length,
            otherExam_count: otherExam.length,
            examCount:
                departmentExam.length + labExam.length + medicalExam.length + otherExam.length,
        });
    }
