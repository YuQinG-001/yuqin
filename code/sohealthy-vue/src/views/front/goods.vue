...
<script lang="ts" setup>
    ...
    async function loadGoodsInfo() {
        ...
        const result = await request.get('/front/goods', { params: { id } });
        if (!result) {
            // router.push({ name: '404' });
            return ElMessage.error('商品不存在');
        }
        // 假设 result 是后端返回的 result
        ...
        data.currentPrice = result.currentPrice ? parseFloat(result.currentPrice) : 0;
        data.originalPrice = result.originalPrice ? parseFloat(result.originalPrice) : 0;
        data.ruleName = result.ruleName ?? '';
        data.packageType = result.packageType ?? '';
        data.tags = Array.isArray(result.tags) ? result.tags.slice() : [];

        // 检查项赋值，如果后端没返回或不是数组，给空数组
        data.departmentExam = Array.isArray(result.departmentExam) ? result.departmentExam : [];
        data.labExam = Array.isArray(result.labExam) ? result.labExam : [];
        data.medicalExam = Array.isArray(result.medicalExam) ? result.medicalExam : [];
        data.otherExam = Array.isArray(result.otherExam) ? result.otherExam : [];

        // 数量赋值（后端 count1~count4 可能不存在，给 0）
        data.departmentExam_count = result.count1 ?? 0;
        data.labExam_count = result.count2 ?? 0;
        data.medicalExam_count = result.count3 ?? 0;
        data.otherExam_count = result.count4 ?? 0;

        // 总数计算
        data.examCount =
            data.departmentExam_count +
            data.labExam_count +
            data.medicalExam_count +
    ...
    async function createPayment() {
        ...
        await ElMessageBox.confirm('您确定要购买该体检套餐吗？', '提示信息', {
            type: 'warning',
        ...
        const sendData = {
            ...
            buyCount: dataForm.number,
        };
        const result: any = await request.post('/front/order/createPayment', sendData);
        dialog.visible = true;
        dialog.qrCode = result.qrCode;
        dialog.outTradeNo = result.outTradeNo;
        const handlePaymentMessage = (envent: any) => {
            const data = JSON.parse(envent.data);
    ...
    async function successHandle() {
        ...
        await request.post('/front/order/paymentResult',sendData);
        proxy?.$socket.removeEventListener('message');
    }
    onMounted(() => {
        loadGoodsInfo();
