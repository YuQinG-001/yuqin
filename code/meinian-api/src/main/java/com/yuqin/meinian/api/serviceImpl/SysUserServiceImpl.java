...
import com.yuqin.meinian.api.mis.DTO.UpdateUserDTO;
...
public class SysUserServiceImpl extends MPJBaseServiceImpl<SysUserMapper, SysUserEntity> implements SysUserService {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public boolean saveUser(@NonNull SysUserEntity sysUserEntity) {
    ...
    public SysUserEntity queryUser(int userId) {
        MPJLambdaWrapper<SysUserEntity> wrapper = JoinWrappers.lambda(SysUserEntity.class)
                .select(SysUserEntity::getUsername, SysUserEntity::getRealName, SysUserEntity::getGender, SysUserEntity::getMobile, SysUserEntity::getEmail, SysUserEntity::getHireDate, SysUserEntity::getRoleIds, SysUserEntity::getDeptId)
                .eq(SysUserEntity::getUserId, userId);
        try {
            return getOneOpt(wrapper, true).orElseThrow(() -> new HisException("无法通过ID查询到用户信息"));
    ...
    private @NonNull MPJLambdaWrapper<SysUserEntity> buildUserWithRolesPageWrapper(@NonNull QueryUserPageDTO dto) {
        MPJLambdaWrapper<SysUserEntity> wrapper = JoinWrappers.lambda("user", SysUserEntity.class)
                .select(SysUserEntity::getUserId, SysUserEntity::getRealName, SysUserEntity::getGender, SysUserEntity::getMobile, SysUserEntity::getHireDate, SysUserEntity::getEmail, SysUserEntity::getUserStatus, SysUserEntity::getIsSuperAdmin)
                ...
                .innerJoin(SysRoleEntity.class, "role", on -> on.apply("JSON_CONTAINS(user.role_ids, CAST(role.role_id AS JSON))"))
                // 动态条件
...
//                .eq(ObjectUtil.isNotNull(dto.getRoleId()), SysRoleEntity::getRoleId, dto.getRoleId())
                ...
                .groupBy(SysUserEntity::getUserId, SysUserEntity::getRealName, SysUserEntity::getGender, SysUserEntity::getMobile, SysUserEntity::getHireDate)
     ...
     * 查询指定用户ID列表中的超级管理员ID
     */
    private List<Integer> querySuperUser(List<Integer> ids) {
        LambdaQueryWrapper<SysUserEntity> superQuery = Wrappers.lambdaQuery(SysUserEntity.class)
                .in(SysUserEntity::getUserId, ids)
                .eq(SysUserEntity::getIsSuperAdmin, 1)
                .select(SysUserEntity::getUserId);
        return baseMapper.selectList(superQuery).stream()
    ...
    @Override
    public int modifyUser(UpdateUserDTO dto) {
        // 使用标准 LambdaUpdateWrapper
    ...
    /**
     * 从原始用户ID列表中过滤掉受保护的用户（当前登录用户 + 超级管理员）
     *
     * @param ids         原始用户ID列表
     * @param loginUserId 当前登录用户ID
     * @return 过滤后可以操作的用户ID列表（不为空，若为空则抛异常）
     */
    private List<Integer> filterValidUserIds(List<Integer> ids, int loginUserId) {
        if (CollectionUtil.isEmpty(ids)) {
            throw new HisException(HttpStatus.BAD_REQUEST.value(), "未选择用户！");
        }
        // 查询这批用户中的超级管理员ID
        List<Integer> superAdminIds = querySuperUser(ids);
        Set<Integer> protectedIds = new HashSet<>();
        protectedIds.add(loginUserId);
        protectedIds.addAll(superAdminIds);
        ...
        if (validIds.isEmpty()) {
            throw new HisException(HttpStatus.BAD_REQUEST.value(), "所选用户中均为当前登录用户或超级管理员，无法操作");
        }
        if (validIds.size() < ids.size()) {
            log.warn("用户 {} 操作受保护用户，已自动过滤。原始 ids: {}, 实际操作 ids: {}", loginUserId, ids, validIds);
        }
        return validIds;
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int removeUserByIds(List<Integer> ids) {
        int loginUserId = StpUtil.getLoginIdAsInt();
        List<Integer> validIds = filterValidUserIds(ids, loginUserId);
        int deletedCount = baseMapper.deleteByIds(validIds);
        if (deletedCount <= 0) {
            throw new HisException(HttpStatus.BAD_REQUEST.value(), "未找到要删除的数据，请刷新后重试");
        }
        return deletedCount;
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int dismissUser(List<Integer> ids) {
        int loginUserId = StpUtil.getLoginIdAsInt();
        List<Integer> validIds = filterValidUserIds(ids, loginUserId);

        // 构造更新对象（空对象，用于触发自动填充）
        SysUserEntity entity = new SysUserEntity();
        entity.setUserStatus(2);  // 离职状态

        LambdaUpdateWrapper<SysUserEntity> wrapper = Wrappers.lambdaUpdate(SysUserEntity.class)
                .in(SysUserEntity::getUserId, validIds);

        int updatedCount = baseMapper.update(entity, wrapper);
        if (updatedCount <= 0) {
            throw new HisException(HttpStatus.BAD_REQUEST.value(), "未找到要离职的用户或用户状态未变更");
        }
        return updatedCount;
    }
